...
export default function DoctorForm({ projectInfo, id }) {
  ...
  const steps = [
    ...
    { id: 3, name: "Record Voice" }, // Add this
    { id: 4, name: "Review" },
  ];
  ...
  const handleAudioUpload = async (e) => {
    ...
    try {
      ...
      const mp3Url = URL.createObjectURL(mp3Blob); // local blob URL
      ...
      setFormData((prevData) => ({
        ...
        doctorDetails: {
          ...prevData.doctorDetails,
          voice: mp3Url,
          voiceBlob: mp3Blob, // store raw blob if needed
        },
      }));

      alert("Audio converted to MP3 successfully!");
    } catch (error) {
      alert("Audio conversion failed. Please try another file.");
    }
  };

  return (
    <div className="h-[100dvh] relative overflow-y-scroll pb-4 w-full scroll-hide">
      {projectInfo &&
        projectInfo.features &&
        projectInfo.features.length > 0 &&
        !projectInfo?.features.includes("default_employee") && (
      ...
      {projectInfo?.top_banner && currentStep == 0 && (
        <div className="flex items-center justify-center w-full max-w-xl mx-auto overflow-hidden"></div>
      )}
      {/* --------------------Header Section----------------- */}
      ...
      <div className=" flex flex-col z-20 px-2 relative overflow-hidden items-center overflow-y-scroll scroll-hide">
        ...
        >
          ...
          <div className="min-w-full flex flex-col items-center rounded-lg">
          <VoiceRecorder setFormData={setFormData}/>
          <NavigationButton
            handlePrevious={handlePrevious}
            currentStep={currentStep}
            steps={steps}
            ...
            <div className="flex flex-col items-center">
              {formData && formData.doctorDetails?.photo && (
                <div className="text-sm font-medium text-gray-800 flex gap-2 mt-2">
                  <div className="w-40 h-40 rounded-xl border-2 border-indigo-500 dark:border-white overflow-hidden justify-center flex"></div>
                </div>
              )}
...
function PhotoUpload({ formData, setFormData, projectInfo }) {
  ...
  const onFileChange = async (e) => {
    ...
    if (file && validateFile(file)) {
      ...
      reader.onload = async () => {
        ...
        // If we should skip cropper, directly upload the image
        if (skipCropper) {
  ...
  const ImagePreview = ({ src, alt, borderClass }) => <div>ImagePreview</div>;
          ...
          <div className="bg-white dark:bg-gray-700 p-4 border-4 border-black w-full">
            ...
            <div className="flex justify-between mt-4 gap-4 px-6">
              <button
                ...
                disabled={isLoading}
              >
                {isLoading ? <ImSpinner3 className="animate-spin" /> : "Cancel"}
              </button>
              <button
                className={`px-4 py-2 ${
