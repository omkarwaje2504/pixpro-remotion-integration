export const FetchProjects = async (dispatch: any) => {
  try {
    const sampleData = {
      id: "49qg2d8r",
      name: "World Hypertension Day AI Video",
      company: "IPCA",
      deliverable: "Hypertension AI video",
      contact_label: "Doctors",
      product_name: "AI Video",
      product_type: "A I Video",
      contacts_limit: 2000,
      employees_count: 0,
      contacts_count: 0,
      downloads_count: 0,
      ready_videos_count: 0,
      created_at: "14/05/2025 05:51 PM",
      closed_at: null,
      texts: {
        login_button: "Login",
        check_employee_login: "Check Employee",
        save_and_next_ecard_button: "Save & Next",
        save_and_next_calendar_button: "Save & Next",
        save_and_add_new_button: "Save & Add New",
        save_and_next_button: "Save & Next",
        contact_label: "Doctor",
        contact_name_label: "Doctor Full Name",
        contact_name_placeholder: "e.g. Dr. Mohit Singh",
        contact_name_hint: "Please enter doctor full name with proper spelling",
        contact_state_label: "Please select the state",
        contact_state_error_message: "Please select the state",
        contact_city_label: "Please select the city",
        contact_city_error_message: "Please select the city",
        contact_state_hint: "eg. Maharashtra",
        contact_city_hint: "eg. Mumbai",
        contact_mobile_label: "Doctor Mobile",
        contact_mobile_placeholder: "e.g. 8899998899",
        contact_mobile_hint:
          "Please enter doctor mobile number, this field is used as unique identification",
        contact_birthday_label: "Doctor Birthday",
        contact_anniversary_label: "Doctor Anniversary",
        employee_password_label: "Password",
        employee_password_placeholder: "e.g. *****",
        employee_password_hint: "Enter password to proceed",
        enter_employee_code_label: "Enter Employee Code",
        employee_code: "Code",
        employee_code_placeholder: "e.g. E-123456",
        employee_code_hint: "Please enter employee code",
        employee_mobile: "Employee Mobile",
        employee_mobile_placeholder: "e.g. 8899998899",
        employee_mobile_hint: "Please enter employee mobile number",
        employee_name: "Name",
        employee_hq: "Employee HQ",
        employee_region: "Employee Region",
        employee_code_to_proceed_error: "Enter employee code to proceed",
        employee_code_or_mobile_to_proceed_error:
          "Enter employee code or mobile to proceed",
        doctor_form_select_language: "Select Language",
        contact_photo_upload: "Contact Photo Upload",
        contact_photo_upload_button:
          "Choose Photo from Gallery or Click Live Photo",
        contacts_limit_reached: "Contacts limit reached!",
        upload_photo_min_1_mb: "Please upload photo of min size 1 MB",
        upload_photo_min_2_mb: "Please upload photo of min size 2 MB",
        download_ecard_button: "Download E-Card",
        preparing_high_quality_image_download:
          "Preparing High Quality Image for Download",
        min_mobile_digits_validation: "10",
        max_mobile_digits_validation: "10",
        mobile_starts_with_validation: "5,6,7,8,9",
        error_message_min_mobile_digits_validation:
          "Please submit 10 digit valid mobile number",
        error_message_max_mobile_digits_validation:
          "Please submit 10 digit valid mobile number",
        error_message_mobile_starts_with_validation:
          "Please submit 10 digit valid mobile number",
        photo_uploading_consent_checkbox_message:
          "I hereby agree to Terms & Conditions and confirm that doctor has given consent to the photo uploading for printing purpose",
        photo_uploading_consent_checkbox_error:
          "Please confirm that doctor has given consent to the photo uploading for printing purpose",
        photo_uploading_consent_popup_title: "Terms & Conditions",
        photo_uploading_consent_popup_message:
          "I hereby confirm that doctor has given consent to the photo uploading for printing purpose",
        photo_uploading_consent_popup_agree_button_message: "Accept",
        calendar_ready_message:
          "Cropping completed, you can proceed with next doctor",
        ecard_add_new_button: "Add New Doctor",
        print_initiated_cannot_update: "Print initiated, cannot update!",
        add_new_button: "Add New Contact",
        save_and_next_video_button: "Save & Proceed",
        download_video_button: "Download Video",
        crop_your_photos_to_see_the_preview:
          "Crop your photo to see the preview",
        doctor_form_select_deliverable: "Select Deliverable",
        contact_email_label: "Doctor Email",
        contact_email_placeholder: "e.g. hello@gmail.com",
        contact_email_hint: "Please enter valid doctor email",
        download_qr_button: "Download QR",
        form_saved_title: "Great!",
        form_saved_successfully: "Details saved, add another doctor!",
        ecard_video_preview_saved_title: "Great!",
        ecard_video_preview_saved_successfully:
          "Details saved, please crop photo on next step!",
        calendar_details_saved_successfully:
          "Details saved, please crop photo on next step!",
        doctor_list_heading_title: "Doctor List",
        project_heading: "Project",
        project_sub_heading: "Project Sub Heading",
        get_started_button: "Get Started",
        login_heading: "Login",
        login_sub_heading: "Login Sub Heading",
        win_section_title: "Win",
        lost_section_title: "Lost",
        spin_prize_accept_button: "Accept Prize",
        spin_prize_reject_button: "Reject Prize",
        contact_country_code_label: "Country Code",
        save_button: "Save",
        duplicate_mobile_no_error_title: "Sorry",
        duplicate_mobile_no_error_msg: "Mobile No Is Repeated",
        preview_audio_text: "Preview Audio",
        ai_script: "Ai Script",
      },
      company_logo: "",
      deliverable_limit: 2000,
      employees_with_contact: null,
      contacts_today: null,
      contacts_yesterday: null,
      employees_today: null,
      employees_yesterday: null,
      artworks: [
        {
          hash: "kmzwmx86l0ye",
          name: "HyperTension",
          thumbnail: null,
          video: null,
          settings: {
            selected_artwork: null,
            canvas_width: null,
            canvas_height: null,
            photo_width: "600",
            photo_height: "600",
            photo_x: null,
            photo_y: null,
            name_font: null,
            name_prefix: null,
            name_x: null,
            name_y: null,
            name_suffix: null,
            name_align: null,
            name_vertical_align: null,
            name_max_width: null,
            name_font_size: null,
            name_font_padding: null,
            name_font_color: "#000000",
            preview_name: null,
            name_arabic: null,
            print_code_font: null,
            print_code_x: null,
            print_code_y: null,
            print_code_align: null,
            print_code_vertical_align: null,
            print_code_font_size: null,
            print_code_font_color: "#000000",
            print_code_rotate: null,
            print_background_rectangle: null,
            print_background_rectangle_color: "#000000",
            print_background_rectangle_padding: null,
            print_background_rectangle_border_width: null,
            print_background_rectangle_border_color: "#000000",
            preview_print_code: null,
            final_resize_width: null,
            final_resize_height: null,
          },
        },
        {
          hash: "z7nlzm1n9odp",
          name: "demo",
          thumbnail: null,
          video: null,
          settings: {
            selected_artwork: null,
            canvas_width: null,
            canvas_height: null,
            photo_width: null,
            photo_height: null,
            photo_x: null,
            photo_y: null,
            name_font: null,
            name_prefix: null,
            name_x: null,
            name_y: null,
            name_suffix: null,
            name_align: null,
            name_vertical_align: null,
            name_max_width: null,
            name_font_size: null,
            name_font_padding: null,
            name_font_color: "#000000",
            preview_name: null,
            name_arabic: null,
            print_code_font: null,
            print_code_x: null,
            print_code_y: null,
            print_code_align: null,
            print_code_vertical_align: null,
            print_code_font_size: null,
            print_code_font_color: "#000000",
            print_code_rotate: null,
            print_background_rectangle: null,
            print_background_rectangle_color: "#000000",
            print_background_rectangle_padding: null,
            print_background_rectangle_border_width: null,
            print_background_rectangle_border_color: "#000000",
            preview_print_code: null,
            final_resize_width: null,
            final_resize_height: null,
            field_1220: [],
            field_1221: [],
            field_1222: [],
          },
        },
      ],
      cropperFields: [
        {
          id: "rjl5pp07go",
          actual: 1217,
          name: "photo",
          label: "Photo",
          hint: null,
          source: null,
        },
      ],
      top_banner:
        "https://pixpro.s3.ap-south-1.amazonaws.com/production/files/2025/05/uHXv2Zzu6ERc52ZaUikACyiq0QFKnRCP3FW85Xpf.jpg",
      bottom_banner: "",
      logo: "https://pixpro.s3.ap-south-1.amazonaws.com/production/files/2025/05/AzTpviowTcVH9zbsFHNGkwcJxovdNfhqyTAXFp2L.jpg",
      features: [
        "enable_ai_data",
        "enable_voice_clone",
        "optional_first_photo",
        "contact_disable_mobile",
      ],
      fields: {
        Doctor_AgeGroup: {
          id: "147pqq9l0n",
          actual: 1220,
          name: "Doctor_AgeGroup",
          label: "Select Age Group",
          type: "dropdown",
          placeholder: null,
          hint: null,
          options: ["26-35", "36-45", "46-60", "60+"],
          default: null,
          validations: {
            required: true,
            min: null,
            max: null,
            max_lines: null,
            max_per_line: null,
          },
          config: {
            hide_csv: false,
            hide_pdf: false,
            edit_not_allowed: null,
            not_equal_to_mobile: null,
            backstage_only: null,
            title_case: null,
            backstage_optional: false,
          },
          helper: null,
        },
        Gender: {
          id: "g6zwmm9lm1",
          actual: 1221,
          name: "Gender",
          label: "Doctor Gender",
          type: "radio",
          placeholder: null,
          hint: null,
          options: ["Male", "Female"],
          default: null,
          validations: {
            required: true,
            min: null,
            max: null,
            max_lines: null,
            max_per_line: null,
          },
          config: {
            hide_csv: false,
            hide_pdf: false,
            edit_not_allowed: null,
            not_equal_to_mobile: null,
            backstage_only: null,
            title_case: null,
            backstage_optional: false,
          },
          helper: null,
        },
        languages: {
          id: "1klxrr0lp5",
          actual: 1222,
          name: "languages",
          label: "Video Language",
          type: "dropdown",
          placeholder: null,
          hint: null,
          options: [
            "English",
            "Hindi",
            "Marathi",
            "Gujarati",
            "Assamese",
            "Bengali",
            "Kannda",
            "Malayalam",
            "Oriya",
            "Tamil",
            "Telugu",
          ],
          default: null,
          validations: {
            required: true,
            min: null,
            max: null,
            max_lines: null,
            max_per_line: null,
          },
          config: {
            hide_csv: false,
            hide_pdf: false,
            edit_not_allowed: null,
            not_equal_to_mobile: null,
            backstage_only: null,
            title_case: null,
            backstage_optional: false,
          },
          helper: null,
        },
      },
      last_contact_at: "28/05/2025 01:04 PM",
      web_link: "https://ipca-pharma.pixpro.app/world-hypertension-day-ipca",
      settings: {
        landing_page: "employee-login",
        form_title:
          "Create Your Personalized AI-Video on Hypertension Awareness",
        token: "ff282b5f-87d5-4836-bc5f-bea55bc5fdc3",
        external_domain: "ai-generator.pixpro.app",
        password: null,
        report_column: "hq",
        approval_stage: null,
        primaryColor: null,
        secondaryColor: null,
        textColor: null,
        ascentColor: null,
        backgroundColor: null,
        customCSS: null,
      },
      seo_title:
        "Hypertension Awareness 2025 – Control Your Blood Pressure, Protect Your Heart",
      seo_description:
        "Watch our special AI-Video for Hypertension Awareness Video. Understand the causes, symptoms, and prevention of high blood pressure",
    };
    return sampleData;
  } catch (e: any) {
    throw new Error("Failed to fetch projects: " + e.message);
  }
};
